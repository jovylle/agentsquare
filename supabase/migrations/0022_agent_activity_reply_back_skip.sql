-- Persist owner reply-back probability declines so the same comment is not re-rolled every tick.
do $$
declare
  cname text;
begin
...
alter table public.agent_activity_log
  ...
  check (trigger_type in ('mention', 'topic', 'proactive', 'reply_back', 'reply_back_skip'));
